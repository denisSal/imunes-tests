...
exteid=`imunes$legacy$debug -b -e pgrj ext.imn | awk '/Experiment/{print $4; exit}'`
startCheck "$exteid"
...
if [ $? -eq 0 ]; then
	...
	if [ $? -eq 0 ]; then
		...
		if [ $? -eq 0 ]; then
			readDump pc1@$exteid eth0 | grep -q 'echo reply'
			if [ $? -ne 0 ]; then
...
if [ $? -ne 0 ]; then
	...
	if [ $? -eq 0 ]; then
		...
		if [ $? -eq 0 ]; then
			readDump pc2@$exteid eth0 | grep -q 'echo reply'
			if [ $? -ne 0 ]; then
				echo "ERROR: no captured 'echo reply' packets"
				err=1
			fi
