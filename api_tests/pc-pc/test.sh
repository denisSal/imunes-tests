...
APIstopNode $eid "pc1"
if [ $? -eq 0 ]; then
	himage pc1@$eid ps > /dev/null 2>&1
	if [ $? -ne 0 ]; then
		APIstartNode $eid "pc1"
		if [ $? -eq 0 ]; then
			himage pc1@$eid ps > /dev/null 2>&1
			if [ $? -ne 0 ]; then
				echo "ERROR: node does not pc1 exist, but it should."
				err=1
			fi
		else
			echo "ERROR: cannot start node pc1."
			err=1
		fi
	else
		echo "ERROR: node pc1 exists, but it should not."
		err=1
	fi
else
	echo "ERROR: cannot stop node pc1."
	err=1
fi
